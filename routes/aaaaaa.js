...
router.post("/projects", async (req, res) => {
  try {
    ...
    // 建立者 id：先用 body 帶來的，沒有就用名字/帳號反查一次
    let creatorId = Number(creator_user_id);
    if (!Number.isFinite(creatorId) && creator_user_name) {
      const r = await pool.query(
        `SELECT id FROM "user" WHERE username = $1 OR name = $1 LIMIT 1`,
        [creator_user_name]
      );
      creatorId = r.rows[0]?.id ?? null;
    }
    ...
    // ✅ 若 stage_id = 1 或 2，強制 start_date 與 estimated_days 必填且有效
    ...
    const params = [
      ...
      creatorId,            // 用算好的 creatorId，而不是 _creator_user_id
      _creator_user_name
    ];
...
router.get("/projects", async (_req, res) => {
  try {
    const sql = `SELECT * FROM project ORDER BY created_at DESC LIMIT 200`;
...
router.patch("/projects/:id", async (req, res) => {
  try {
    ...
    // 讀取舊資料以判斷 due_date 是否要重算
    const { rows: oldRows } = await pool.query(`SELECT stage_id, start_date, estimated_days FROM project WHERE id=$1`, [idNum]);
    if (oldRows.length === 0) return res.status(404).json({ ok: false, message: "專案不存在" });
    const prev = oldRows[0];

    const next_stage_id       = patch.stage_id       !== undefined ? patch.stage_id       : prev.stage_id;
    const next_start_date     = patch.start_date     !== undefined ? patch.start_date     : prev.start_date;
    const next_estimated_days = patch.estimated_days !== undefined ? patch.estimated_days : prev.estimated_days;

    // // 若更新後 stage_id = 1，強制 start_date 與 estimated_days 必填且有效
    // if (Number(next_stage_id) === 1 || Number(next_stage_id) === 2) {
    //   if (!next_start_date || !dayjs(next_start_date).isValid()) {
    //     return res.status(400).json({
    //       ok: false,
    //       code: "REQUIRE_START_DATE",
    //       message: "stage_id=1 時，start_date 必填且需為有效日期(YYYY-MM-DD)"
    //     });
    //   }
    ...
    //       ok: false,
    //       code: "REQUIRE_ESTIMATED_DAYS",
    //       message: "stage_id=1 時，estimated_days 必須為正數"
    //     });
    //   }
    // }
